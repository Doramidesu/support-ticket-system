    ...
    <form @submit.prevent="handleSubmit" class="p-6 space-y-6">
      ...
      <div>
        ...
        <p class="text-xs text-gray-500 mt-1">
          Jika diisi, kamu akan menerima notifikasi status tiket via WhatsApp.
        </p>
        <div v-if="error?.phone" class="flex items-center mt-2">
          <p class="text-xs text-red-500">{{ error.phone[0] }}</p>
        </div>
      </div>
