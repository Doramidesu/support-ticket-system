...
async function connectToWhatsApp() {
  ...
  puskom.ev.on('messages.upsert', async (m) => {
    ...
    if (/^(status|cek)/i.test(t)) {
      ...
      if (!code) {
        await sendTextSimple(puskom, sender, 'Format salah.\nContoh: status KP-20260312-A8D4F2')
        return
      }
...
Ketik *menu* untuk kembali ke menu utama.`
    ...
    if (checkTimeout(phone)) {
      await sendTextSimple(puskom, sender, '⏳ Sesi pengaduan berakhir (15 menit). Ketik *menu* untuk mulai ulang.')
    }
    ...
    if (state.state === 'awaiting_akademik') {
      if (t === '1' || /^kpst$/i.test(t)) {
        ...
        await sendTextSimple(puskom, sender, formTemplateFor('KPST') + "\n\nKetik 'batal' jika salah.")
        return
      }

      if (t === '2' || /^krs$/i.test(t)) {
        setUserState(phone, 'awaiting_form', 'KRS')
        await sendTextSimple(puskom, sender, formTemplateFor('KRS') + "\n\nKetik 'batal' jika salah.")
        return
      }

      await sendTextSimple(puskom, sender, 'Pilihan tidak valid.\nBalas *1* untuk KPST atau *2* untuk KRS.')
      return
    }
    ...
    if (state.state === 'awaiting_form') {
      if (/^batal$/i.test(t)) {
        clearUserState(phone)
        await sendTextSimple(puskom, sender, 'Proses dibatalkan. Kembali ke menu.')
...
Terima kasih telah menghubungi PUSKOM WICIDA 🙏`
    ...
    // ========== greeting ==========
    if (/^(halo|hi|hello|menu|selamat pagi|selamat siang|selamat sore)$/i.test(t)) {
      await sendTextSimple(puskom, sender, greetingMessage(pushname))
      return
    }
    ...
    if (/^[1-6]$/.test(t)) {
      if (t === '1') {
        ...
        await sendTextSimple(puskom, sender, formTemplateFor('WeLearn') + "\n\nKetik 'batal' jika salah.")
        return
      }
      if (t === '2') {
        setUserState(phone, 'awaiting_form', 'SIak')
        await sendTextSimple(puskom, sender, formTemplateFor('SIak') + "\n\nKetik 'batal' jika salah.")
        return
      }
      if (t === '3') {
        setUserState(phone, 'awaiting_form', 'Email')
        await sendTextSimple(puskom, sender, formTemplateFor('Email') + "\n\nKetik 'batal' jika salah.")
        return
      }
      if (t === '4') {
      ...
      if (t === '5') {
        ...
        await sendTextSimple(puskom, sender, formTemplateFor('Puskom') + "\n\nKetik 'batal' jika salah.")
        return
      }
      if (t === '6') {
        await sendTextSimple(
...
*Tiket Saya*`
    ...
    // fallback
    await sendTextSimple(puskom, sender, `Halo *${pushname}*, ketik *menu* untuk memulai.`)
  })
}
